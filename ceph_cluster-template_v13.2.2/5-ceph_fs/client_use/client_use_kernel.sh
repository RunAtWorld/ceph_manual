...
# sudo mount -t ceph ceph-rc-2:6789:/ /mnt/ceph_kernel -o name=admin,secret=AQA0qGlbiRSPDRAALdqWHkWrOhmPwaCUY5nA2Q==  #使用密钥环字符串方式
sudo mount -t ceph ceph-rc-2:6789:/ /mnt/ceph_kernel  -o name=admin,secretfile=/etc/ceph/admin.secret ##使用指定密钥环文件的方式，admin.secret文件中是一个base64编码的字符串

sudo umount /mnt/ceph_kernel #卸载

#MOUNT CEPH FS USING FUSE
sudo mkdir -p /etc/ceph
sudo scp ceph@ceph-rc-1:/etc/ceph/ceph.conf /etc/ceph/ceph.conf  # 从集群中拉取keyring, Client 默认读 /etc/ceph/ceph.conf 
sudo scp ceph@ceph-rc-1:/etc/ceph/ceph.client.admin.keyring /etc/ceph/keyring  # 从集群中拉取keyring, Client 默认读/etc/ceph/ceph.client.admin.keyring,
																			#/etc/ceph/ceph.keyring,/etc/ceph/keyring,/etc/ceph/keyring.bin
																			#4个文件只要有一个就可
sudo chmod a+r /etc/ceph/ceph.conf #修改权限为所有人可读
sudo chmod a+r /etc/ceph/keyring   #修改权限为所有人可读

sudo yum -y install ceph-fuse
#mount a CephFS as fuse 
sudo mkdir /mnt/ceph_fuse
sudo chown ceph: /mnt/ceph_fuse
sudo ceph-fuse -m ceph-rc-2:6789 /mnt/ceph_fuse
...
sudo ceph-fuse -m ceph-rc-2:6789 /mnt/ceph_fuse2
...
ceph-rc-2:6789:/     /mnt/ceph    ceph    name=admin,secretfile=/etc/ceph/secret.key,noatime,_netdev    0       2
EOM
